struct Light
{
    ...
    float4 directionAndType; // 0 directional, 1 point, 2 spot
    float4 colorAndIntensity;
    float4 spotAngles;       // inner cosine, outer cosine
    float4 padding;
};
...
cbuffer LightingConstants : register(b0)
{
    ...
    float4 gShadowParameters; // inverse map size, minimum bias, slope bias, unused
...
float4 PSMain(VSOutput input) : SV_TARGET
{
    ...
    color = color / (1.0f + color); // Reinhard tone mapping
